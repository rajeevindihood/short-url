...
class psycopg2_hdlr:
    ...
    def connect(self):
        try:
            self.conn = psycopg2.connect(database=self.db, user=self.user, password=self.passwd, host=self.host, port=self.port, cursor_factory = psycopg2.extras.DictCursor)
            self.conn.autocommit = True
            self.cursor = self.conn.cursor()
        except psycopg2.OperationalError as err:
            log.critical("-- <<DB-POOL>> Database Error during connection setup => %s --",str(err),exc_info=1)
            self.err=err
...
class dbHdlr:
    def __init__(self):
        ...
        for i in range(1,int(os.getenv("DATABASE_POOL_SIZE"))+1):
            dbHndlr_obj = psycopg2_hdlr(os.getenv("DATABASE_HOST"), os.getenv("DATABASE_USER"), os.getenv("DATABASE_PASSWORD"), os.getenv("DATABASE_NAME"), int(os.getenv("DATABASE_PORT")),i)
