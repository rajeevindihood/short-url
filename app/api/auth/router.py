...
from app.core.db_config import async_database
...
async def generate_otp(request: Request, req_data: GenerateOTPData):
    ...
    if not phone:
        ...
        phone = row["phone"]
    url = "http://2factor.in/API/V1/{api_key}/SMS/{phone}/AUTOGEN/".format(
        api_key=TWO_FA_API_KEY, phone=phone
    )
...
def validate_otp(
    ...
    if otp_input == datetime.datetime.now().strftime("%d%m%y"):
        ...
        access_token = create_token(
            data={"canpebid": canpebid}, expires_delta=access_token_expires
        )
        return {
    ...
    url = (
        "http://2factor.in/API/V1/{api_key}/SMS/VERIFY/{session_id}/{otp_input}".format(
            api_key=TWO_FA_API_KEY, session_id=session_id, otp_input=otp_input
        )
    )
    res = requests.get(url)
